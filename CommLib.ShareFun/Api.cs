...
namespace HXJL.CommLib.ShareFun
{
    ...
    public class Api
    {
        ...
        /// <param name="lpWindowName">窗体名字，用spy++看</param>
        /// <returns></returns>
        ...
        /// <param name="lpszClass">子窗体的类名 为""表示所有类  </param>
        /// <param name="lpszWindow">子窗体的标题 为""表示要查找的窗体无标题 如空白的textBox控件  </param>
        ...
        /// <param name="lParam">附加的消息指定信息(字符串类型，这里直接传字符串就好了，用于给输入框输入内容)</param>
        /// <returns></returns>
        ...
        /// <param name="Msg">消息</param>
        /// <param name="wParam">附加的消息指定信息(当发送键盘消息时此值为键盘按键的十六进制虚拟键盘)</param>
        /// <param name="lParam">附加的消息指定信息(当发送鼠标消息时此值为点击客户区逻辑坐标值，为x+(y*0x10000))</param>
        /// <returns></returns>
        [DllImport("User32.dll", EntryPoint = "SendMessage")]
        ...
        /// <param name="nCmdShow">显示参数，SW开头的参数，SW_MAXIMIZE，SW_SHOWNORMAL</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 找到指定窗口下的对应子窗口句柄，自动遍历到最底层（用递归）
        /// </summary>
        /// <param name="main">指定窗口</param>
        /// <param name="childName">子窗口名（用spy++看）</param>
        /// <returns></returns>
        public static IntPtr GetChild(IntPtr main, string childName)
        {
            //先看看子窗口有没有。。。。
            IntPtr rightChild = FindWindowEx(main, IntPtr.Zero, null, childName);
            ...
            do
            {
                child = FindWindowEx(main, child, null, null);
                ...
                if (child != IntPtr.Zero)
                {
                    ...
                    //看看找到的子窗口里面有没有，没有就继续
                    IntPtr rc = GetChild(child, childName);
                    if (rc != IntPtr.Zero)
                    {
                        return rc;
                    }
        ...
        /// <summary>
        /// 找到指定窗口句柄下的第几个子窗口--在没有子窗口名时用
        /// </summary>
        /// <param name="main">指定窗口句柄</param>
        /// <param name="retCnt">第几个子窗口</param>
        /// <returns>返回指定数量的子窗口句柄，如果没有的话，那就直接返回IntPtr.Zero了</returns>
        public static IntPtr GetChildNo(IntPtr main, int retCnt)
        {
            IntPtr child = IntPtr.Zero;
        ...
        /// </summary>
        /// <param name="main">指定窗口</param>
        /// <param name="retCnt">子窗口序列，使用int数组</param>
        /// <returns></returns>
        public static IntPtr GetChildSerial(IntPtr main, int[] retCnt)
        {
            IntPtr retPtr = main;

            //循环找就行了。。
            for (int i = 0; i < retCnt.Length; i++)
            {
                retPtr = GetChildNo(retPtr, retCnt[i]);
        ...
        /// <param name="cnt">点击数量</param>
        /// <param name="intval">每次点击间隔时间</param>
        public static void SendClickEvent(IntPtr handle, int x, int y, int cnt, int intval = 10)
        {
            int cordinate = y * 0x10000 + x;

            for (int i = 0; i < cnt; i++)
            {
                SendMessage(handle, APICode.WM_LBUTTONDOWN, 0, cordinate);
                Thread.Sleep(1);
                SendMessage(handle, APICode.WM_LBUTTONUP, 0, cordinate);
                Thread.Sleep(intval);
            }
        }
        #endregion
